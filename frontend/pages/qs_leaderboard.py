...
def show():
    ...
    company_id = co_map.get(selected_co)
    flag_param = "" if selected_flag == "All Flags" else selected_flag

    leaderboard = api.get_qs_leaderboard(company_id=company_id, flag=flag_param)
    ...
    st.dataframe(df.drop(columns=["_id"]), use_container_width=True, height=480, hide_index=True)

    st.divider()
    st.subheader("Score Comparison Chart")

    chart_data = [
        {"name": r["Name"], "score": r["Overall (/10)"]}
        for r in rows[:15]
    ]
    if chart_data:
